/*
 * @(#)DirectoryManager.java	1.28 06/04/07
 *
...
public class DirectoryManager {
    ...
    public static String getDirectoryPath(String packageName) {
        ...
        if (pathStr.length() > 0 && pathStr.charAt(pathStr.length() - 1) != File.separatorChar)
    ...
    public static String getPath(String name) {
        return name == null || name.length() == 0 ? "" : name.replace('.', URL_FILE_SEPARATOR);
    }
    ...
    public static String getRelativePath(String from) {
        if (from == null || from.length() == 0)
            return "";
        return from.replaceAll("[^\\.]*\\.?", ".." + URL_FILE_SEPARATOR);
    }
    ...
    public static void createDirectory(Configuration configuration, String path) {
        ...
        if(!dir.exists() && !dir.mkdirs()) {
        	configuration.message.error("doclet.Unable_to_create_directory_0",
					path);
