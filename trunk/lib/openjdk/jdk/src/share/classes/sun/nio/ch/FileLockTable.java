...
class SharedFileLockTable extends FileLockTable {
    ...
    private static class FileLockReference extends WeakReference<FileLock> {
        private org.example.FileKey fileKey;

        FileLockReference(FileLock referent,
                          ReferenceQueue<FileLock> queue,
                          org.example.FileKey key) {
            super(referent, queue);
            this.fileKey = key;
        }

        org.example.FileKey fileKey() {
            return fileKey;
        }
    }

    // The system-wide map is a ConcurrentHashMap that is keyed on the org.example.FileKey.
    ...
    private static ConcurrentHashMap<org.example.FileKey, List<FileLockReference>> lockMap =
        new ConcurrentHashMap<org.example.FileKey, List<FileLockReference>>();
    ...
    private final org.example.FileKey fileKey;

    SharedFileLockTable(Channel channel, FileDescriptor fd) throws IOException {
        this.channel = channel;
        this.fileKey = org.example.FileKey.create(fd);
    }
    ...
    private void removeKeyIfEmpty(org.example.FileKey fk, List<FileLockReference> list) {
    ...
    private void removeStaleEntries() {
        ...
        while ((ref = (FileLockReference)queue.poll()) != null) {
            org.example.FileKey fk = ref.fileKey();
